...
func (p *Processor) Start() error {
	...
	if p.statusError {
		return errors.New("Processor execution failed")
	}
...
func (p *Processor) createTriggers(processorConfiguration *processor.Configuration) ([]trigger.Trigger, error) {
	...
	completeChan := make(chan *controlcommunication.ControlMessage)
	abstractControlMessageBroker.Subscribe(controlcommunication.ControlMessageKind("complete"), completeChan)
	go p.listenOnCompleteMessage(completeChan)
...
func (p *Processor) listenOnCompleteMessage(controlMessageChan chan *controlcommunication.ControlMessage) {
	for streamCompleteControlMessage := range controlMessageChan {

		p.logger.DebugWith("Received explicit complete control message", "controlMessage", streamCompleteControlMessage)

		// retrieve attributes from control message
		explicitCompleteAttributes := streamCompleteControlMessage.Attributes

		status := explicitCompleteAttributes["status"].(bool)
		// TODO decide on status value
		p.logger.DebugWith("Status", "status", status)
		p.statusError = status
		p.terminateAllTriggers(syscall.SIGTERM)
		p.Stop()
	}
}
